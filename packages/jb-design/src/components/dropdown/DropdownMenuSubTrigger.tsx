...
>(({ className, inset, rightChevron, rightChevronClassName, children, ...props }, ref) => {
  const renderRightChevron = useMemo(() => {
    if (rightChevron === false) {
      return null;
    }
    if (!isBoolean(rightChevron) && !isUndefined(rightChevron)) {
      return <span className={cn('jb-chevron-right', rightChevronClassName)}>{rightChevron}</span>;
    }
    return (
